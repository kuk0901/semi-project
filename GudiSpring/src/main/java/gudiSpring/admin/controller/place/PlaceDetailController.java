...
@WebServlet(value = "/admin/place/detail") 
public class PlaceDetailController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			int placeNo = Integer.parseInt(getPartAsString(req.getPart("placeNo")));
			...
			String phWebsite = getPartAsString(req.getPart("plWebsite"));
			...
			PlaceDto placeDto = new PlaceDto(placeNo, placeName, plAddress, plPhone, phWebsite, fileName);
			...
			int result;
			result = placeDao.placeUpdate(placeDto);

			if (result > 0) {
				req.setAttribute("msg", "장소 정보를 변경했습니다.");
			} else if (result == 0) {
				req.setAttribute("msg", "장소 내용을 변경하지 못했습니다.");
			}
